...
var QueryPanel = React.createClass({
    ...
    onChangeQueryType: function (type) {
        this.setState({query_box: type});
    },

    onQuery: function (query_data) {
        //alert(query_data);
        query_data.page = 1;
        query_data.size = 16;
    ...
    doQueryPackage: function (query_data) {
    ...
    doQueryBox: function (query_data) {
        query_data.card_type = CARD_TYPE;
        $.ajax({
            url: '/card/query/box',
            dataType: 'json',
    ...
    onQueryCard: function (requ_type, box_info, status, page) {
    ...
    onCardMaintain: function (operation, type, id) {
            ...
            fullNode = (
                <div className="row">
                    <div className="col-md-12 col-lg-6">
                        <OperationPanel onchange={this.onChangeQueryType} doQuery={this.onQuery}
                                        query_box={this.state.query_box}/>
                        <PackageResultPanel onpage={this.onPage} package_data={this.state.package_data}/>
                    </div>
                </div>
            )
        } else {
            fullNode = (
                <div className="row">
                    <div className="col-md-12 col-lg-6">
                        ...
                        <BoxResultPanel onpage={this.onPage} oncard={this.onQueryCard} box_data={this.state.box_data}
                                        onCardMaintain={this.onCardMaintain}
                        />
                    </div>

                    <div className="col-md-12 col-lg-6">
                        <CardResultPanel oncard={this.onQueryCard}
                    ...
                    <div className="modal-dialog">
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <h4 className="modal-title" id="myModalLabel">修改卡密</h4>
                            </div>
                            <div className="modal-body">
                                <div className="form-group">
                                    <label className="col-md-3 control-label">盒号</label>

                                    <div className="col-md-9">
                                        <input id="box_id" type="text" readOnly="true"
                                               className="m-bot15 form-control input-sm"/>
                                    </div>
                                    <label className="col-md-3 control-label">卡号</label>

                                    <div className="col-md-9">
                                        <input id="card_id" type="text" readOnly="true"
                                               className="m-bot15 form-control input-sm"/>
                                    </div>
                                    <label className="col-md-3 control-label">新的卡密</label>

                                    <div className="col-md-9">
                                        <input id="password" type="text" className="m-bot15 form-control input-sm"/>
                                    </div>

                                    <div className="col-md-offset-3 col-md-9">
                                        <div className="checkbox">
                                            <label>
                                                <input type="checkbox" id="no_modify"/>
                                                不修改密码，直接设置为可用
                                            </label>
                                        </div>
                                    </div>
                            ...
                            <div className="modal-footer">
                                <a href="javascript:void(0);" className="btn btn-default" data-dismiss="modal">关闭</a>
                                <a href="javascript:void(0);" onClick={this.onModifyCard}
                                   className="btn btn-danger">修改</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="panel-body">
                    <form className="form-horizontal" method="get">
                        <div className="form-group">
                            ...
                            <label className="col-md-2 control-label">入库时间</label>
                            ...
                            <div className="col-md-4 m-bot15">
                                <input id="form_package" type="text" className="form-control input-sm"/>
                            </div>

                            <label className="col-md-2 control-label">卡号</label>
                            ...
                            <label className="col-md-2 control-label">查询方式</label>

                            <div className="col-md-4 m-bot15 checkbox">
                                <label>
                                    <input id="query-type" type="checkbox" onClick={this.onChangeType}
                            ...
                            <div className="col-md-2">
                                <a id="act_export_error" href="javascript:void(0);" className="btn btn-danger"
...
var BoxResultPanel = React.createClass({
    getBoxStatusStr: function (status) {
        var box_status_map = {
            'ready': '在用',
            'frozen': '冻结',
            'removed': '已删除',
    ...
    render: function () {
        ...
        var box_rows = this.props.box_data.data.map(function (box, i) {
            var query_error_card_btn = (<button className='btn btn-default btn-xs' onClick={this.onCard.bind(this, box) }>查错卡</button>);

            var freeze_btn = null;
            if (box.status == 'ready' && box.used < box.count)
            {
                freeze_btn = (<button className='btn btn-default btn-xs'
            ...
            {
                remove_btn = (<button className='btn btn-default btn-xs'
            ...
            return (
                <tr>
                    <td>{box.package_id}</td>
                    <td>{box.price}</td>
                    <td>{box.filename}</td>
                    <td>{box.create_time}</td>
                    <td>{box.count}</td>
                    <td>{box.ready}</td>
                    <td>{box.inuse}</td>
                    <td>{box.used}</td>
                    <td>{box.error}</td>
                    <td>{ this.getBoxStatusStr(box.status) }</td>
                    <td>{ box.status_time}</td>
                    <td>
                        {query_error_card_btn} {freeze_btn} {remove_btn}
                    </td>
                </tr>
            );
        }.bind(this));
        ...
        return (
            <section className="panel">
                ...
                <div className="table-responsive">
                    <table id="box_result" className="table table-hover">
                        <thead>
                        <tr>
                            <th>箱号</th>
                            <th>面值</th>
                ...
                <div className="row">
                    <div className="col-sm-12">
                        <div className="btn-row dataTables_filter">
                            <div id="page_group1" className="btn-group">
                                {page_group}
                            </div>
                        </div>
                    </div>
...
var PackageResultPanel = React.createClass({
    ...
    render: function () {
        ...
        var package_rows = this.props.package_data.data.map(function (package_info, i) {
            ...
            return (
                <tr>
                    ...
                    <td>{package_info.start}-{package_info.end}</td>
                    <td>{package_info.price}</td>
                    <td>{package_info.c}</td>
                    <td>{package_info.r}</td>
                </tr>
            );
        });
        ...
        return (
            <section className="panel">
                <div>
                    共{this.props.package_data.package_count}箱
                    {this.props.package_data.box_count} 盒
                    {this.props.package_data.total_count}张
                    面值合计 {this.props.package_data.total_sum}元
                    其中可用{this.props.package_data.ready_count}张，可用面值合计{this.props.package_data.ready_sum}元
                </div>
...
var CardResultPanel = React.createClass({
    getCardStatusStr: function (status) {
        var card_status_map = {
            'ready': '可用',
            'inuse': '在用',
            'error': '错误',
    ...
    onClickExportErrorCards: function () {
        ...
        for (var i in requ_data.argu_list) {
            argu_list += _.str.sprintf('&%s=%s',
                ...
                encodeURIComponent(requ_data.argu_list[i])
            )
        }

        $.ajax({
            url: _.str.sprintf('/api/card/error_cards?&requ_type=%s%s',
    ...
    onEdit: function (box_id, card_id) {
        $('#card_id').val(card_id);
        $('#box_id').val(box_id);
        $('#password').val('');
    ...
    componentDidUpdate: function (prevProps, prevState) {
        if (this.props.box_info != prevProps.box_info) {
            ...
            $('#upload_correction_cards').fileupload(
                'option',
                {
                    url: _.str.sprintf('/upload/correction_box_file?card_type=%s&box_id=%s',
                        encodeURIComponent(CARD_TYPE),
                        encodeURIComponent(this.props.box_info.box_id)
                    )
                }
            );
        }
    ...
    render: function () {
        var search_bar = [
            { status: 'all',      name: '全部'},
            { status: 'inuse',    name: '在用'},
            { status: 'ready',    name: '可用'},
            { status: 'error',    name: '错误'},
            { status: 'used',     name: '已用'},
            { status: 'frozen',   name: '冻结'},
            { status: 'removed',  name: '已删除'},
        ].map(function (s) {
            if (s.status == this.props.query.status) {
                return (
                    <a href="#" className="btn btn-primary"
                       onClick={this.onCard.bind(this, s.status, undefined)}>{s.name}<span className="badge">{this.props.card_data.count}</span></a>
                );
            } else {
                return (
                    <a href="#" className="btn btn-default"
                       onClick={this.onCard.bind(this, s.status, undefined)}>{s.name}</a>
                );
            }
        }.bind(this));
        ...
        var card_rows = this.props.card_data.data.map(function (card, i) {
            ...
            if (card.status == 'error') {
                btn = (<a className="btn btn-xs btn-danger" href="javascript:void(0);"
            ...
            return (
                <tr>
                    ...
                    <td>{card.price}</td>
                    <td>{card.card_id}</td>
                    <td>{this.getCardStatusStr(card.status)}</td>
                    <td>{card.status_time}</td>
                    <td>
                        {btn}
                    </td>
                </tr>
        ...
        var boxInfoNode = (
            <span>箱号:{this.props.box_info.package} 盒号:{this.props.box_info.box_id} 文件名:{this.props.box_info.filename}</span>
        );
        return (
            <section className="panel">
                ...
                <div className="panel-body">
                    ...
                    <div className="col-md-6 btn-group">
                        ...
                        <a className="btn btn-default fileinput-button"
                        >
                ...
                <div className="table-responsive">
                    <table id="card_result" className="table table-hover">
                        <thead>
                        <tr>
                            <th>编号</th>
                            <th>面值</th>
